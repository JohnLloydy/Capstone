...
export class UserPage {
  ...
  save() {
    ...
    if (this.userForm.valid) {
      if (this.mode === 'create') {
        ...
        newitem.name =
          newitem.firstname + ' ' + newitem.middlename + ' ' + newitem.lastname;
        ...
        updateditem.name =
          ...
          ' ' +
          updateditem.middlename +
          ' ' +
          updateditem.lastname;

        this.userService.crudService
        ...
        'Cancel',
        {
          ...
          handler: (data: any) => {
            if (data.password !== data.confirmpassword) {
              ...
              this.userService.crudService
                .updateEntity(this.user.id, userupdate)
                .subscribe((res) => {
                  return true;
                });
                return true;
  ...
  async ionViewDidEnter() {
    ...
    if (this.userid) {
      await this.userService.crudService
        .fetchEntity(this.userid)
        .subscribe(async (item) => {
          if (item) {
            ...
            if (item.region_code) {
              ...
              await this.getMunicipalities(item.province_code).subscribe(
                (result) => {
              ...
              await this.getBarangays(item.municipality_code).subscribe(
                (result) => {
  ...
  printQR(item : any) {
    ...
    if (popupWinindow) {
      ...
      popupWinindow.document.write(
        '<html><head><link rel="stylesheet" type="text/css" href="style.css" /></head><body onload="window.print()"><div style="border-style:solid; border-width:0.5px; padding:1px; position:absolute; width:250px;"><div style="float:left; width:250px;"><center><p style="margin:0px; font-size:8px;"><u>' +
          item.name +
          ' </u></p><p style="margin:0px; font-size:8px;">' +
